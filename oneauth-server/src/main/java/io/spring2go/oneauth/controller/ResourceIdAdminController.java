...
import io.spring2go.oneauth.dao.entity.ClientDetailsToResourceIdXrefEntity;
...
public class ResourceIdAdminController {
    ...
    @RequestMapping(path = "/_remove/{resId}", method = RequestMethod.GET, produces = {MediaType.TEXT_HTML_VALUE, MediaType.APPLICATION_XHTML_XML_VALUE})
    public String remove(@PathVariable("resId") String resId, RedirectAttributes attributes) {

        resourceIdRepository.findOneByValue(resId).map(
                resourceIdEntity -> {
