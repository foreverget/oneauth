...
import io.spring2go.oneauth.dao.entity.ClientDetailsToAuthorizedGrantTypeXrefEntity;
...
public class GrantTypeAdminController {
    ...
    @RequestMapping(path = "/_remove/{grantTypeName}", method = RequestMethod.GET, produces = {MediaType.TEXT_HTML_VALUE, MediaType.APPLICATION_XHTML_XML_VALUE})
    public String removeGrantType(@PathVariable("grantTypeName") String grantTypeName, RedirectAttributes attributes) {

        grantTypeRepository.findOneByValue(grantTypeName.toLowerCase()).map(
                grantTypeEntity -> {
