package io.spring2go.oneauth.service;

import io.spring2go.oneauth.dao.entity.*;
import io.spring2go.oneauth.dao.repository.*;
...
public class OAuth2DatabaseClientDetailsService implements ClientDetailsService, ClientRegistrationService {
    ...
    public ClientDetails loadClientByClientId(String clientId) throws ClientRegistrationException {
        return clientDetailsRepository.findOneByClientId(clientId).map(entityToDomain)
                .<ClientRegistrationException>orElseThrow(() -> new NoSuchClientException("Client ID not found"));
    }
    ...
    public void addClientDetails(ClientDetails clientDetails) throws ClientAlreadyExistsException {
        ...
        clientDetailsEntity.setRedirectUris(clientDetails.getRegisteredRedirectUri().stream().map(
    ...
    public void updateClientDetails(ClientDetails clientDetails) throws NoSuchClientException {
        ...
        ClientDetailsEntity entity = clientDetailsRepository.findOneByClientId(clientDetails.getClientId())
        ...
        // merge grant type
        Set<ClientDetailsToAuthorizedGrantTypeXrefEntity> grantTypeXrefEntityRemoves = entity.getAuthorizedGrantTypeXrefs().stream().filter(
                grantTypeXrefEntity -> !clientDetails.getAuthorizedGrantTypes().contains(grantTypeXrefEntity.getGrantType().getValue())
        ).collect(Collectors.toSet());
        ...
        Set<ClientDetailsToAuthorizedGrantTypeXrefEntity> grantTypeXrefEntityNewOnes = clientDetails.getAuthorizedGrantTypes().stream()
                .filter(grantType -> !grantTypeOriginValueSet.contains(grantType)).map(
                        grantType -> grantTypeRepository.findOneByValue(grantType).map(
                                grantTypeEntity -> ClientDetailsToAuthorizedGrantTypeXrefEntity.builder()
        ...
        // merge scopes
        Set<ClientDetailsToScopesXrefEntity> scopeXrefEntityRemoves = entity.getScopeXrefs().stream().filter(
                clientDetailsToScopesXrefEntity -> !clientDetails.getScope().contains(clientDetailsToScopesXrefEntity.getScope().getValue())
        ).collect(Collectors.toSet());

        Set<String> scopeOriginValueSet = entity.getScopeXrefs().stream().map(xref -> xref.getScope().getValue()).collect(Collectors.toSet());
        Set<ClientDetailsToScopesXrefEntity> scopeXrefEntityNewOnes = clientDetails.getScope().stream()
                .filter(scope -> !scopeOriginValueSet.contains(scope)).map(
                        scope -> scopeRepository.findOneByValue(scope).map(
                                scopeEntity -> ClientDetailsToScopesXrefEntity.builder()
                                        .clientDetails(entity)
                                        .scope(scopeEntity)
        ...
        // merge resource id
        Set<ClientDetailsToResourceIdXrefEntity> resourceIdXrefEntityRemoves = entity.getResourceIdXrefs().stream()
                .filter(xref -> !clientDetails.getResourceIds().contains(xref.getResourceId().getValue())).collect(Collectors.toSet());
        ...
        Set<ClientDetailsToResourceIdXrefEntity> resIdXrefEntityNewOnes = clientDetails.getResourceIds().stream().filter(resId -> !resIdOriginValueSet.contains(resId)).map(
                resId -> resourceIdRepository.findOneByValue(resId).map(
                        resourceIdEntity -> ClientDetailsToResourceIdXrefEntity.builder()
                                .clientDetails(entity)
                                .resourceId(resourceIdEntity)
                                .build()
                ).<ClientRegistrationException>orElseThrow(() -> new ClientRegistrationException("Unknown resource id: " + resId))
        ).collect(Collectors.toSet());
        ...
        // merge redirect uri
        Set<RedirectUriEntity> redirectUriEntityRemoves = entity.getRedirectUris().stream().filter(
                redirectUriEntity -> !clientDetails.getRegisteredRedirectUri().contains(redirectUriEntity.getValue())
        ).collect(Collectors.toSet());

        Set<String> originRedirectUrisValue = entity.getRedirectUris().stream().map(RedirectUriEntity::getValue).collect(Collectors.toSet());
        Set<RedirectUriEntity> redirectUriEntityNewOnes = clientDetails.getRegisteredRedirectUri().stream().filter(
                redirectUri -> !originRedirectUrisValue.contains(redirectUri)).map(
                redirectUri -> RedirectUriEntity.builder()
                        .clientDetails(entity)
                        .value(redirectUri)
                        .build()
    ...
    public void updateClientSecret(String clientId, String secret) throws NoSuchClientException {
        ClientDetailsEntity clientDetailsEntity = clientDetailsRepository.findOneByClientId(clientId)
                .<NoSuchClientException>orElseThrow(() -> new NoSuchClientException("Client id not found."));
    ...
    public void removeClientDetails(String clientId) throws NoSuchClientException {
        ...
        ClientDetailsEntity entityToRemove = clientDetailsRepository.findOneByClientId(clientId)
                .<NoSuchClientException>orElseThrow(() -> new NoSuchClientException("Client id not found."));
